...
export default function AmDetailPage() {
  const { article, content, prevArticle, nextArticle } = useArticleDetail('am');

  if (!article) {
    return (
      <div className="article-detail-page" style={{ textAlign: 'center', padding: '80px 0' }}>
        <p>기사를 찾을 수 없습니다.</p>
        <Link to="/article/amList" style={{ color: 'var(--color-accent)', marginTop: '16px', display: 'inline-block' }}>
          목록으로 돌아가기
        </Link>
      </div>
    );
  ...
  return (
    <div className="article-detail-page">
      ...
      <nav className="article-nav">
        {prevArticle ? (
          <Link to={`/article/amDetail?idx=${prevArticle.idx}`} className="article-nav-link article-nav-link--prev">
            <span className="article-nav-label">&larr; 이전 글</span>
            <span className="article-nav-title">{prevArticle.title}</span>
          </Link>
        ) : <div />}
        {nextArticle ? (
          <Link to={`/article/amDetail?idx=${nextArticle.idx}`} className="article-nav-link article-nav-link--next">
